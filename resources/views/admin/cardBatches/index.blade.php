...
@can('card_batch_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.card-batches.create') }}">
                ...
                <tbody>
                    @foreach($cardBatches as $key => $cardBatch)
                        <tr data-entry-id="{{ $cardBatch->id }}">
                            ...
                            <td>
                                @can('card_batch_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.card-batches.show', $cardBatch->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('card_batch_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.card-batches.edit', $cardBatch->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('card_batch_delete')
                                    <form action="{{ route('admin.card-batches.destroy', $cardBatch->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('card_batch_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.card-batches.massDestroy') }}",
